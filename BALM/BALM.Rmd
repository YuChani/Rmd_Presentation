...
## Appendix (Proof Theorem2)
.nt-02.f-85[
  - $\mathbf{u}_m, \mathbf{u}_n$을 constatnt로 본다. $\mathbf{F}_{m,n}^{\mathbf{p}_j}$의 계산을 쉽게 하는 형태로 바꾸는것
...
$$\frac{\partial(\mathbf{u}_m^T \mathbf{A} \mathbf{u}_n)}{\partial \mathbf{p}_j} = 
\frac{1}{N} \sum_{i=1}^{N} \frac{\partial}{\partial \mathbf{p}_j} \left[ ((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_m) ((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_n) \right]$$
$$= \frac{1}{N} \sum_{i=1}^{N} \left[ ((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_m) \frac{\partial((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_n)}{\partial \mathbf{p}_j} + 
((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_n) \frac{\partial((\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{u}_m)}{\partial \mathbf{p}_j} \right]$$
- $\mathbf{u}_m, \mathbf{u}_n$을 constatnt로 보고있으므로.
...
$$\sum_{i=1}^{N} (\mathbf{p}_i - \bar{\mathbf{p}}) = 0$$
- 최종적으로 
$$\frac{\partial(\mathbf{u}_m^T \mathbf{A} \mathbf{u}_n)}{\partial \mathbf{p}_j} = 
\frac{(\mathbf{p}_j - \bar{\mathbf{p}})^T}{N} (\mathbf{u}_m \mathbf{u}_n^T + \mathbf{u}_n \mathbf{u}_m^T)$$
...
.nt-02.f-85[
  - 첫번째 항
$$\mathbf{u}_k \mathbf{u}_k^T \frac{\partial(\mathbf{p}_i - \bar{\mathbf{p}})}{\partial \mathbf{p}_j}$$
- 두번째 항
$$\mathbf{u}_k (\mathbf{p}_i - \bar{\mathbf{p}})^T \mathbf{U} \mathbf{F}_k^{\mathbf{p}_j}$$
- 세번째 항
$$\mathbf{U} \mathbf{F}_k^{\mathbf{p}_j} (\mathbf{u}_k^T (\mathbf{p}_i - \bar{\mathbf{p}}))$$
]
---
## Appendix (Proof Theorem2)
.nt-02.f-90[
  - $\mathbf{p}_i$에 대한 미분이 $i=j$인 경우와 $i \neq j$인 경우로 나뉘어서 계산
$$\frac{\partial(p_i - \bar{p})}{\partial p_j} = 
\begin{cases} 
